...
def build_rag_chat_router() -> APIRouter:
    ...
    async def post_message_stream(
        ...
        try:
            hybrid_request = request.model_copy(update={"retrieval_mode": "hybrid"})
            retrieval, stream_events = await orchestrator.collect_evidence(
                session_id=session_id,
                request=hybrid_request,
            )
        except Exception as exc:  # pragma: no cover
        ...
        async def token_stream() -> AsyncIterator[str]:
            # Phase 1: Emit intermediate events (one JSON object per line)
            for stream_event in stream_events:
                yield stream_event.model_dump_json() + "\n"
