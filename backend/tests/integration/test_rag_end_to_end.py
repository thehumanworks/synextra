...
async def test_rag_ingestion_persistence_and_chat(client: AsyncClient) -> None:
    fixture = Path(__file__).parent.parent / "fixtures" / "1706.03762v7.pdf"
    data = await fixture.read_bytes()
...
@pytest.mark.asyncio
async def test_document_ingestion_accepts_text(client: AsyncClient) -> None:
    response = await client.post(
        "/v1/rag/documents",
    ...
    body = response.json()
    assert body["document_id"]
    assert body["mime_type"] in {"text/markdown", "text/plain"}
