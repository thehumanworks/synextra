...
export function CitationAccordion({
  ...
  useEffect(() => {
    ...
    const elementId = buildCitationReferenceElementId(effectiveScopeId, focusReferenceIndex);
  ...
  }, [effectiveScopeId, focusReferenceIndex, onFocusReferenceHandled]);
  ...
  const selectedCitation =
    selectedCitationIndex == null ? null : citations[selectedCitationIndex] ?? null;
  ...
  return (
    <div className="mt-4">
      ...
      >
        <span>
          Used {sourceReferenceCount} source{sourceReferenceCount === 1 ? "" : "s"}
        </span>
        <span
          aria-hidden="true"
          className={cn(
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        Answer references use [n] and match the source tags below.
      </p>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <div className="flex flex-wrap items-center gap-1">
                        {entry.referenceIndices.map((referenceIndex, referenceIdx) => {
                          const highlighted = focusReferenceIndex === referenceIndex;
                          return (
                            <span
                              key={referenceIndex}
                            ...
                            >
                              ...
                              {referenceIdx < entry.referenceIndices.length - 1 ? (
          ...
          >
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-xs text-muted-foreground">
                    References {formatReferenceIndices(selectedCitation.referenceIndices)}
                  </div>
              ...
              <div className="mt-3 text-xs text-muted-foreground">
                Source: <span className="font-mono">{selectedCitation.citation.source_tool}</span>
              </div>
            </motion.div>
          </motion.div>
        ) : null}
