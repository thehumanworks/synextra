...
- OpenAI SDK env wiring for orchestration/search/persistence must support both direct OpenAI and Azure-compatible setups: key via `OPENAI_API_KEY` (alias `AZURE_OPENAI_API_KEY`) and optional endpoint override via `OPENAI_BASE_URL` (aliases `AZURE_OPENAI_BASE_URL` / `AZURE_OPENAI_ENDPOINT`).
...
- `RagChatRequest` no longer exposes `retrieval_mode` as a runtime setting; backend/SDK retrieval is always hybrid. The request validator strips legacy `retrieval_mode` payloads for backward compatibility.
...
- The orchestrator no longer instantiates `OpenAI` or `AsyncOpenAI` clients directly. All LLM interaction goes through the `openai-agents` SDK (`Agent` + `Runner`). The SDK manages its own client internally; OpenAI-compatible providers can be selected via env (`OPENAI_BASE_URL`, optional `SYNEXTRA_OPENAI_API=chat_completions`).
