...
run_dev_server() {
  ...
  if command -v setsid >/dev/null 2>&1; then
    setsid env PYTHONWARNINGS="ignore" uv run uvicorn synextra_backend.app:app --reload --log-level error &
    backend_pid=$!
    backend_pgid="${backend_pid}"
  else
    env PYTHONWARNINGS="ignore" uv run uvicorn synextra_backend.app:app --reload --log-level error &
    backend_pid=$!
  fi
